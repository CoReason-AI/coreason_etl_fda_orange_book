...
from coreason_etl_fda_orange_book.silver.transform import transform_products
...
def mock_source() -> MagicMock:
    """Fixture for mocking FdaOrangeBookSource."""
...
def test_read_permission_error(mock_source: MagicMock, tmp_path: Path) -> None:
...
def test_file_locked(mock_source: MagicMock, tmp_path: Path) -> None:
...
def test_disk_full(tmp_path: Path) -> None:
...
def test_memory_exhaustion_transform(tmp_path: Path) -> None:
    """
