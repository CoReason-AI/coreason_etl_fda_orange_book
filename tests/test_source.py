...
def test_extract_archive_zip_slip_exception(fda_source: FdaOrangeBookSource, tmp_path: Path) -> None:
    ...
    # Mock os.path.commonpath to raise ValueError (simulating cross-drive on Windows or similar)
    with patch("os.path.commonpath", side_effect=ValueError("Test error")):
        files = fda_source.extract_archive(zip_path, extract_dir)
