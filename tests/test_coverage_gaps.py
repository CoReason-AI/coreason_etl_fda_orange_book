...
class TestCoverageGaps:
    ...
    def test_silver_ingestion_hints(self, tmp_path: Path):
        """Test different filename hints in ingestion."""
        # Setup files with specific names
        otc = tmp_path / "otc.txt"
        otc.write_text("Ingredient~Trade_Name~Applicant~Strength~Appl_No~Product_No~TE_Code~Approval_Date~RLD\nI~T~A~S~001~001~~Jan 1, 2020~No", encoding="utf-8")

        disc = tmp_path / "disc.txt"
        disc.write_text("Ingredient~Trade_Name~Applicant~Strength~Appl_No~Product_No~TE_Code~Approval_Date~RLD\nI~T~A~S~001~001~~Jan 1, 2020~No", encoding="utf-8")
    ...
    def test_gold_ingestion_hints_and_concats(self, tmp_path: Path):
        """Test gold ingestion aggregation branches."""
        # Setup valid product files
        p1 = tmp_path / "rx.txt"
        p1.write_text("Ingredient~Trade_Name~Applicant~Strength~Appl_No~Product_No~TE_Code~Approval_Date~RLD\nI~T~A~S~001~001~~Jan 1, 2020~No", encoding="utf-8")

        # Setup valid patent file
        pat = tmp_path / "patent.txt"
        pat.write_text("Appl_No~Product_No~Patent_No~Patent_Expire_Date_Text~Drug_Substance_Flag~Drug_Product_Flag~Patent_Use_Code~Delist_Flag\n001~001~123~Jan 1, 2030~Y~N~U~N", encoding="utf-8")
        ...
        otc = tmp_path / "otc.txt"
        otc.write_text("Ingredient~Trade_Name~Applicant~Strength~Appl_No~Product_No~TE_Code~Approval_Date~RLD\nI2~T2~A~S~002~001~~Jan 1, 2020~No", encoding="utf-8")

        disc = tmp_path / "disc.txt"
        disc.write_text("Ingredient~Trade_Name~Applicant~Strength~Appl_No~Product_No~TE_Code~Approval_Date~RLD\nI3~T3~A~S~003~001~~Jan 1, 2020~No", encoding="utf-8")

        files_map = {
            "products": [p1, otc, disc],
            "patent": [pat],
            "exclusivity": [exc]
        }
    ...
    def test_silver_empty_dataframe_continue(self, tmp_path: Path):
